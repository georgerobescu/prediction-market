...
class ConnectionModal extends React.Component {
      ...
      <React.Fragment>
        ...
        <Box mt={4} mb={5}>
          ...
          <Text fontSize={[2, 3]} my={3}>
            Connecting lets you use the FCLA & Gnosis Geospatial Prediction
            Market via your Ethereum account.
          </Text>
        </Box>
        ...
        >
          <Box flex={"1 1"} width={1} mt={[3, 0]} mb={[4, 0]} mr={4}>
            ...
            <Text fontSize={1}>
              Your Ethereum account activity is public on the blockchain. Choose
              an account you don’t mind being linked with your activity here.
            </Text>
          </Box>
          <Box flex={"1 1"} width={1} mt={[3, 0]} mb={[4, 0]} mr={4}>
            <Flex justifyContent={"center"} mb={3}>
              <Icon name="AccountBalanceWallet" color="primary" size="4rem" />
            </Flex>
          ...
          <Box flex={"1 1"} width={1} mt={[3, 0]} mb={[4, 0]}>
            <Flex justifyContent={"center"} mb={3}>
              <Icon name="People" color="primary" size="4rem" />
            </Flex>
  ...
  renderConnectButton = () => {
    if (GeneralUtil.hasMetaMask()) {
      ...
      return (
        <Button onClick={this.props.validateAccount} width={[1, "auto"]}>
          Connect
        </Button>
      );
    }
      ...
      <Modal isOpen={this.props.isOpen}>
        <ModalCard closeFunc={this.props.closeModal}>
          {this.state.showTxFees === false ? (
            <React.Fragment>
              <ModalCard.Body>{this.renderModalContent()}</ModalCard.Body>
              <ModalCard.Footer>{this.renderConnectButton()}</ModalCard.Footer>
            </React.Fragment>
          ) : (
            <ModalCard.Body>
              <TransactionFeeModal />
